...
def get_arguments():
    ...
    group_input_data.add_argument('-r', '--reference',
                                  help='Single file with reference genome in FASTA format OR *.txt file with '
                                       'one-per-line list of FASTA files with reference sequences', type=str, nargs='+')

    group_input_data.add_argument('--gtf', help='File with gene coordinates. We recommend to use files downloaded from GENCODE '
                                       'or Ensembl [GTF/GFF]', type=str, nargs='+')
...
def transform_to_meta_arguments(args, tmp_dir, logger):
    ...
    if args.reference and len(args.reference) > 1:
        for i_path in range(len(args.reference)):
            args.reference[i_path] = add_id_to_chrs_in_fasta(args.reference[i_path], tmp_dir, str(i_path))

    if args.gtf and len(args.gtf) > 1:
        for i_path in range(len(args.gtf)):
            args.gtf[i_path] = add_id_to_chrs_in_gtf(args.gtf[i_path], tmp_dir, str(i_path))

    args.reference = get_join_file(args.reference, tmp_dir, 'reference', 'fa', logger)

    args.gtf = get_join_file(args.gtf, tmp_dir, 'gene_database', 'gtf', logger)

    return args


def get_join_file(paths, outdir, name, ext, logger):
...
def add_id_to_chrs_in_gtf(in_path, tmp_dir, addition_str):
...
def add_id_to_chrs_in_fasta(in_path, tmp_dir, addition_str):
